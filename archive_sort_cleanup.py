...
if __name__ == '__main__':
    ...
    for date_dir in date_directories:
        ...
        for sensor_dir in sensor_dirs:
            ...
            filename_spec = np.zeros(len(sensor_dir_content))
            for i, psbl_rawfile in enumerate(sensor_dir_content_names):
                if psbl_rawfile == 'SA_log.out':
                    filename_spec[i] = 0
                    continue
                ...
                if len(rawfile_split) == 2:
                    ...
                    this_file_cpu = int(rawfile_split[7], 16)
                    this_file_relay = rawfile_split[8]
                elif len(rawfile_split) == 6:
                    # This is a current file with no GPS... see https://github.com/wx4stg/Bruning_Slow_Antenna_Software/issues/3    ['20240815', '102012', '875419', '0.00', '93af7e18', 'a'
                    filename_spec[i] = 3
                    this_file_dt = dt.strptime(rawfile_split[0]+rawfile_split[1]+rawfile_split[2], '%Y%m%d%H%M%S%f')
                    this_file_lat = None
                    this_file_lon = None
                    ...
                    this_file_cpu = int(rawfile_split[4], 16)
                    this_file_relay = rawfile_split[5]
                    new_file_name = f'{this_file_dt.strftime("%Y%m%d_%H%M%S_%f")}_NO_FIX_2Donly_{this_file_gps_err}_{this_file_cpu:x}_{this_file_relay}.raw'
